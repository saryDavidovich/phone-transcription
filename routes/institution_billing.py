...
# כתובות המקור הרשמיות של עדכוני ה-CallBack (מהתיעוד) - לוג אזהרה אם עדכון
# מגיע ממקום אחר, לא חסימה קשיחה (בהתאם להמלצת נדרים פלוס עצמם).
...
def topup():
    ...
    if amount < 5:
        return jsonify({'error': 'סכום מינימלי לחיוב הוא 5 ש"ח'}), 400
...
def nedarim_callback(charge_id):
    ...
    if ip not in NEDARIM_CALLBACK_IPS:
        log.warning(f'Nedarim callback from unexpected IP: {ip} (charge_id={charge_id})')
