...
# קידומת תגובה/העברה גנרית: מילה קצרה (עברית/לועזית) ואז ':' - במקום רשימה
# סגורה מראש (Re/Fwd/תגובה/הועבר בלבד), כדי לתפוס גם קידומות אחרות שתוכנות
# מייל שונות משתמשות בהן (למשל "תשובה:" בהגדרות עברית של Outlook, "AW:"/"SV:"
# בלקוחות לא-עבריים, וכו') בלי צורך למנות כל שפה/תוכנה בנפרד. תג בסוגריים
# מרובעים כמו "[EXTERNAL]" שחלק משערי דואר ארגוניים מוסיפים - מוסר גם הוא.
_REPLY_PREFIX_RE = re.compile(r'^\s*(\[[^\]]{1,20}\]\s*)?[A-Za-z\u0590-\u05FF]{1,12}\s*:\s*')
...
def _preprocess_image_for_ocr(img_bytes):
    """
    עיבוד תמונה לשיפור OCR - נוסחה שנבדקה אמפירית מול כמה חלופות (כולל חידוד/בינריזציה/CLAHE)
    ויצאה הכי טובה: היפוך צבעים + מתיחת ניגודיות (ללא clipping).
    טקסט לבן על רקע שחור, בניגודיות מקסימלית, בלי "לשרוף" קווי עט דקים.
    ...
    try:
        ...
        # היפוך צבעים (טקסט לבן על רקע שחור) + מתיחת ניגודיות מלאה, בלי חיתוך קצוות
...
def _gemini_ocr(filepath, original_filename):
    ...
    try:
        ...
        OCR_PROMPT = """אתה סורק OCR מכני לכתב יד עברי בלבד (לא דפוס, לא כתב רש"י) - בד"כ תוכן תורני. אינך מבין עברית, רק מעתיק צורות אותיות כמו מצלמה.
...
<p style="line-height:1.8">
ב<b>שורת הנושא</b> של המייל כותבים את מספר הטלפון שלך:<br>
